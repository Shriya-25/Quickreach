/**
 * QuickReach — One-time staff seeding script
 *
 * Prerequisites:
 *   1. cd scripts
 *   2. npm install
...
async function seedStaff() {
  ...
  for (const staff of staffList) {
    const ref = db.collection('staff').doc(staff.staffId);
    batch.set(ref, staff);
    console.log(`  ✓ Queued: ${staff.name} (${staff.role})`);
  }

  await batch.commit();

  console.log(`\n✅ Done! ${staffList.length} staff members seeded to Firestore.`);
  process.exit(0);
}
